...
parser.add_argument("-p", "--password", default='admin', help="User's password for MIMIC Database", type=str)
pargs = parser.parse_args()

# Initializing database connection
conn = pg.connect("dbname='mimic' user={0} host='localhost' options='--search_path=public' password={1}".format(pargs.username,pargs.password))
...
from public.microbiologyevents
"""
...
from public.admissions ad, public.icustays i, public.patients p, public.elixhauser_quan elix
...
for i in range(0,100000,10000):
  ...
  public.chartevents where icustay_id>="+str(200000+i)+" and icustay_id< " + str(210000+i) + " and value is not null and \
...
) as xx inner join  public.labevents as f on f.hadm_id=xx.hadm_id and f.charttime>=xx.intime-interval '1 day' 
...
from public.outputevents oe, public.icustays ic
...
from public.inputevents_mv ie, public.d_items ci
...
(
select ie.icustay_id, sum(ie.amount) as sum
from public.inputevents_cv ie, public.d_items ci
where ie.itemid=ci.itemid and ie.itemid in (30054, 30055, 30101, 30102, 30103, 30104, 30105, 30108, 226361,
226363, 226364, 226365, 226367, 226368, 226369, 226370, 226371, 226372, 226375, 226376, 227070, 227071, 227072)
group by icustay_id
)
